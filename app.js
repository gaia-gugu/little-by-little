...
function refreshCompanionUI() {
 for(const el of document.querySelectorAll('.companion-presence'))el.outerHTML=companionPresence();
 const question=$('.question-companion');if(question)question.outerHTML=companionPresence(true);
...
function companionPresence(question=false) {
...
const navIcons={home:'m3 11 9-8 9 8v10h-6v-7H9v7H3z',choose:'M5 4h14v16H5zM8 8h8M8 12h2m4 0h2M8 16h2m4 0h2',challenge:'m13 2-8 12h6l-1 8 9-13h-6z',learn:'M4 14v-3a8 8 0 0 1 16 0v3M4 12h4v8H4zM16 12h4v8h-4z',progress:'M4 20V12m8 8V8m8 12V4'};
function renderNavigation() {
 if(!$('.settings-open'))$('.masthead').insertAdjacentHTML('beforeend',button('<span aria-hidden="true">⚙</span><span>Settings</span>','settings','settings-open','aria-label="Settings"'));
 ...
 const active=view==='home'?'home':view==='learn'?'learn':view==='progress'?'progress':['companions','settings'].includes(view)?'':['challenge','bests','best-detail'].includes(view)||(['practice','results'].includes(view)&&state.round?.mode==='challenge')?'challenge':'choose';
 $('.bottom-nav').innerHTML=[['home','Home'],['choose','Practice'],['learn','Listen'],['progress','Progress']].map(([route,label])=>`<a href="#${route}" data-route="${route}"${active===route?' aria-current="page"':''}><svg class="nav-icon" viewBox="0 0 24 24" aria-hidden="true"><path d="${navIcons[route]}"/></svg>${label}</a>`).join('');
}
function discloseHelp() {
 const options={choose:['Practice help','.guided-panel p, .settings > p.small'],challenge:['Challenge help','.challenge-settings > .intro + p, .challenge-settings > p.small'],learn:['Listening help','.audio-page > p.small:not([id])']};
 const option=options[view];if(blocked||!option)return;
 const [label,selector]=option,paragraphs=[...$('#main').querySelectorAll(selector)];
 if(!paragraphs.length)return;
 const details=document.createElement('details');details.className='how-it-works';
 const summary=document.createElement('summary');summary.textContent=label;details.append(summary,...paragraphs);
